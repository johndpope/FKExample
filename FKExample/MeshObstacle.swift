...
class MeshObstacle {
    ...
    */
    ...
    func debugVerticies() {
        if let scene = self.sprite.scene as? GameScene,
            let debug = scene.childNode(withName: "PermanentDebugLayer") {
            ...
            let outline = SKShapeNode(path: self.path)
            outline.zPosition = 1001
            outline.lineWidth = 1
            outline.position = CGPoint(x:1024, y:768)
